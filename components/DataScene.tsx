...
export default function DataScene({ runKey, running }: { runKey:number; running:boolean }) {
  ...
  useEffect(()=>{
    ...
    // Packets are anchored to the same spline as the visible pipeline. Small clustered
    // phase offsets create batches without allowing particles to drift away from the line.
    const count=180, arr=new Float32Array(count*3), phases=new Float32Array(count), speeds=new Float32Array(count);
    for(let i=0;i<count;i++){
      const batch=Math.floor(i/12), within=i%12;
      phases[i]=((batch/15)+(within-5.5)*.0018+Math.random()*.0015)%1;
      speeds[i]=.026+Math.random()*.009;
      const v=curve.getPoint((phases[i]+1)%1);arr[i*3]=v.x;arr[i*3+1]=v.y;arr[i*3+2]=v.z;
    }
    const pg=new THREE.BufferGeometry();pg.setAttribute("position",new THREE.BufferAttribute(arr,3));const particles=new THREE.Points(pg,new THREE.PointsMaterial({color:0xd8b876,size:.045,transparent:true,opacity:.72,sizeAttenuation:true}));group.add(particles);
    ...
    let frame=0;const clock=new THREE.Clock();const animate=()=>{frame=requestAnimationFrame(animate);const t=clock.getElapsedTime();group.rotation.y+=(mouse.x*.09-group.rotation.y)*.025;group.rotation.x+=(-.12+mouse.y*.045-group.rotation.x)*.025;nodes.forEach((n,i)=>{n.rotation.x=t*(.18+i*.025);n.rotation.y=t*(.24+i*.02);n.position.y=positions[i][1]+Math.sin(t*1.1+i)*.035});
      ...
      for(let i=0;i<count;i++){
        const progress=(phases[i]+t*speeds[i])%1;
        const v=curve.getPoint(progress);
        attr.setXYZ(i,v.x,v.y,v.z);
      }
      attr.needsUpdate=true;
      renderer.render(scene,camera)};animate();state.current={scene,camera,renderer,particles,frame,mouse,nodes};
